 ...
 Copyright (c) 2011, 2022, Oracle and/or its affiliates. All rights reserved.
   ...
   NAME
     ...
     Release 22.3.0 README file


  Contents
  ========
    Supported Software Versions
    What's new on Oracle Graph Server and Client 22.3.0
    What's new on Oracle Graph Server and Client 22.2.0
    What's new on Oracle Graph Server and Client 21.4.3
    What's new on Oracle Graph Server and Client 20.4.6
  ...
  Main new features on Oracle Graph Server and Client 22.3.0:

   - PGX.ML Enhancements
      - Added support for GraphWise on partitioned praphs
      - Added support for ARM platforms
   - Graph Server improvements
      - Show filter expressions in PGQL explain plan
      - Added support for OnMissingVertex option for readGraphByName and CREATE PROPERTY GRAPH APIs
      - Added support for INTERVALs in PGQL
      - Added support for ONE ROW PER STEP in PGQL
      - Added support for path visualization (in GraphViz) using ONE ROW PER STEP
      - Added keystore support for HTTPS setup
      - Made underlying PARALLEL hint configurable when loading from PG views and partitioned graphs
      - Added fetchone, fetchmany, fetchall support on OPG4Py result set
   - PGQL on PG Views improvements
      - Added hint to influence path translation
      - Added configurable graph metadata cache and translation cache
   - RDF Query UI web application improvements
      - Added management of Subject Property Matrix (SPM) auxiliary tables for speeding up SPARQL query execution
      - Optimized SPARQL query caching to improve translation from SPARQL into SQL

  The following problems were fixed in Oracle Graph Server and Client 22.3.0:

   - CVE-2022-29885 in 3rd party component Apache Tomcat
   - CVE-2022-25647 in 3rd party component Gson
   - BUG 34083005: AdbGraphClientConfiguration doesn't show methods in Javadoc
   - BUG 34111360: Wrong tabular values in PgqlDriver graph result
   - BUG 34114860: Geo Layout is not working properly
   - BUG 34129422: ADB client sometimes throwing 500 when requesting env status
   - BUG 34146473: GraphViz: count(*) query throws NoSuchElementException
   - BUG 34151779: PGX Server log gets appended instead of creating a new file when restarting
   - BUG 34151951: SELECT fails if property alias is equals to vertex/edge variable name
   - BUG 34205296: Graph Server returns invalid username/password if error code is ORA-12505
   - BUG 34227963: Wrong table result when using PGQL functions/aggregations
   - BUG 34260169: PGQL on RDBMS: throws an error if property name is a DB reserved word
   - BUG 34300635: GraphViz is not marking FunctionCall as selected on properties tooltip
   - BUG 34334892: GraphViz: remove dotted line for path queries, use ONE ROW PER STEP instead to visualize paths

  What's new on Oracle Graph Server and Client 22.2.0
  ===================================================

  Main new features on Oracle Graph Server and Client 22.2.0:

   - Graph Server improvements
      - Replaced Log4J logging with Logback
      - Added support for loading subgraphs from PG Views
      - Added support for ELEMENT_NUMBER and MATCH_NUMBER functions for ONE ROW PER EDGE/VERTEX
      - Added optimized_for_read, optimized_for_updates and synchronizable options for readGraphByName(name, PG_VIEW)
   - PGQL on PG Views improvements
      - Added support for INSERT/UPDATE/DELETE
      - Added support for metadata graph
      - Added support for Path finding goal ALL
   - Added a client for ADB Graph REST API (ADB Graph Client)
   - PGX.ML Enhancements
      - Added support for weight decay in GraphWise
      - Added support for user-configurable options to GNNExplainer
   - RDF Server improvements
      - Added support for public page in RDF web application
      - Added RDF Turtle format support
      - Added Hyperlinks support on RDF SPARQL queries table view
      - Added REST service to return RDF query plan

  The following problems were fixed in Oracle Graph Server and Client 22.2.0:

    - CVE-2022-23181 in 3rd party component Apache Tomcat
    - CVE-2020-36518 in 3rd party component Jackson Databind
    - CVE-2021-4048 in 3rd party component OpenBLAS / JavaCPP
    - CVE-2021-22569 in 3rd party component Protocol Buffers
    - BUG 33730025: Sharing a graphic graphviz fails
    - BUG 33958546: Graphviz not displaying full graph with 1000 results per page
    - BUG 33195641: Unable to grant file permissions using Autonomous Database
    - BUG 33441039: GraphViz throws NullPointerException if /graphs is not called before executing a query
    - BUG 33499688: ORA-01704: string literal too long for variable-length path query
    - BUG 33581137: GraphViz fails if graph server is behind load balancer
    - BUG 33585376: PG_VIEW: Edge pattern returns one result even though the graph doesn't have any edges
    - BUG 33730155: Graphviz: invalid expansion of vertices
    - BUG 33734152: NPE in GraphViz when accessing shared graph
    - BUG 34067523: ONE ROW PER EDGE throws NPE in GraphViz
    - BUG 33741017: GraphViz throws 500 if the pgql driver specified during login does not exist
    - BUG 33764856: Authorization rules for files not working on ADB
    - BUG 33835954: Sombrero Server fails when port is occupied without a helpful error message
    - BUG 33891682: OPG4Py does not close the underlying connection
    - BUG 33935768: Table visualization not working due to change of formatter
    - BUG 33982298: embedding graphviz as iframe not working

  What's new on Oracle Graph Server and Client 21.4.3
  ===================================================

  This is the third limited patch cycle release on top of version 21.4.0
  ...
  This is the sixth limited patch cycle release on top of version 20.4.0
  which contains fixes for the following issues:
  ...
  This is the second limited patch cycle release on top of version 21.4.0
  which contains fixes for the following issues:
  ...
  This is the first patch release on top of version 21.4.0 which contains fixes for the following issues:
